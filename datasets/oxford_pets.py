...
class OxfordPets(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        if not cfg.DATALOADER.SELECTION:
            train, val, test = self.subsample_classes(train, val, test, subsample=subsample)
        else:
            train = self.get_specific_class(train, label = cfg.DATASET.CUR_IDX, num_sample = 100)
            ...
            for arg in args:
                for item in arg:
                    if item.label == idx:
                        ...
                        idx += 1
        return ordered_classnames
    
    @staticmethod
    def get_specific_class(dataset, label, num_sample = 100):
        selected_set = []
        for item in dataset:
            if item.label == label:
                selected_set.append(item)
            if len(selected_set) >= num_sample:
                break
        return selected_set
    ...
    @staticmethod
    def get_attrs(all_classattrs, subsample="all"):
        if subsample == 'all':
            return all_classattrs
        
        m = math.ceil(len(all_classattrs) / 2)
        if subsample == 'base':
            classattrs = all_classattrs[:m]
        else:
            classattrs = all_classattrs[m:]
        return classattrs